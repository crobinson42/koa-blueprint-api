...
function getControllerMethodCustomRouteList({ controller, method }) {
  ...
  else if (!isObject(_config.routes))
    throw new Error('config/routes.js must export an object')

  const ctrlMethodCustomRoutes = getCtrlMethodCustomRouteConfig(
    _config.routes,
    controller,
    method,
  )

  if (!ctrlMethodCustomRoutes.length) return undefined

  debug(
    `Loaded ${
      ctrlMethodCustomRoutes.length
    } custom route for ${controller}.${method}`,
  )
