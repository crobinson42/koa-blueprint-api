...
const { Adapter, reserved } = require("js-data-adapter");
...
const DEFAULTS = {
  ...
  castMapperNormalIdAttribute: true,
...
Adapter.extend({
  ...
  _swapIdAttribute(r, opts) {
    if (this.getOpt("swapIdAttribute", opts)) {
      if (Array.isArray(r)) {
        r.forEach(_r => {
          if (_r._id) {
            _r.id = _r._id;
    ...
    const _checkFields = r => {
      for (let field in r) {
        if (r[field]._bsontype === "ObjectID") {
          r[field] =
            typeof r[field].toString() === "string"
              ? r[field].toString()
              : r[field];
        }
      }
    ...
    return this._run((client, success, failure) => {
      ...
      client
        ...
        .countDocuments(
          mongoQuery,
          countOpts,
          (err, count) => (err ? failure(err) : success([count, {}]))
        );
    });
  },
    ...
    return this._run((client, success, failure) => {
      ...
      client
        ...
        .insertMany(
          props,
          insertManyOpts,
          (err, cursor) => (err ? failure(err) : success(cursor))
        );
    }).then(cursor => {
      let records = [];
      let r = cursor.ops ? cursor.ops : cursor;
  ...
  _destroy(mapper, id, opts) {
    ...
    return this._run((client, success, failure) => {
      ...
      const mongoQuery = {
        [this._getIdAttribute(mapper, opts)]: this.toObjectID(mapper, id)
      };
      const collection = client.collection(collectionId);
      const handler = (err, cursor) => (err ? failure(err) : success(cursor));
  ...
  _find(mapper, id, opts) {
    ...
    return this._run((client, success, failure) => {
      ...
      client
        ...
        .findOne(
          mongoQuery,
          (err, record) => (err ? failure(err) : success(record))
        );
    }).then(record => {
      if (record) {
        this._translateObjectIDs(record, opts);
        this._swapIdAttribute(record, opts);
      } else {
        record = undefined;
      }
  ...
  _findAll(mapper, query, opts) {
    ...
    return this._run((client, success, failure) => {
      ...
      this._swapIdAttribute(records, opts);
      return [records, {}];
    });
  },
    ...
    return this._find(mapper, id, { raw: false })
      .then(result => {
        ...
        return this._run((client, success, failure) => {
          ...
          const collection = client.collection(collectionId);
          const handler = (err, cursor) =>
            err ? failure(err) : success(cursor);
      ...
      .then(cursor => {
        return this._find(mapper, id, { raw: false }).then(result => {
          cursor.connection = undefined;
  ...
  _updateAll(mapper, props, query, opts) {
    ...
    return this._run((client, success, failure) => {
      return this._findAll(mapper, query, { raw: false }).then(result => {
        ...
        ids = result[0].map(record =>
          this.toObjectID(mapper, record[this._getIdAttribute(mapper, opts)])
        );
      ...
      const query = {
        [this._getIdAttribute(mapper, opts)]: {
          in: ids
        }
      };
      return this._findAll(mapper, query, { raw: false }).then(result => {
  ...
  toObjectID(mapper, id) {
    const validIdKey =
      mapper.idAttribute === "_id" ||
      (mapper.idAttribute === "id" &&
        this.getOpt("castMapperNormalIdAttribute", this._opts));
  ...
  makeBelongsToForeignKey(mapper, def, record) {
    return this.toObjectID(
      def.getRelation(),
      Adapter.prototype.makeBelongsToForeignKey.call(this, mapper, def, record)
    );
  },
  ...
  makeHasManyLocalKeys(mapper, def, record) {
    ...
    const localKeys = Adapter.prototype.makeHasManyLocalKeys.call(
      this,
      mapper,
      def,
      record
